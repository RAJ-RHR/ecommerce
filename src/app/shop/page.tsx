  ...
  return (
    <div className="mt-20 px-4 md:px-8">
      <Head>
        ...
        <meta name="keywords" content="Herbolife, herbal products, supplements, health, ayurveda, wellness store, shop" />
        ...
        <meta property="og:description" content="Explore Herbolife's full range of wellness and herbal health products. 100% genuine, fast delivery, easy returns." />
      ...
      <div className="flex overflow-x-auto hide-scrollbar gap-2 mb-6 py-2">
        <button
          onClick={() => setCategory('')}
          className={`px-4 py-1 rounded-full border text-sm whitespace-nowrap ${category === '' ? 'bg-green-600 text-white' : 'border-gray-400 text-gray-700'}`}
        >
          All
        </button>
        {categories.map((cat) => (
          <button
            ...
            className={`px-4 py-1 rounded-full border text-sm whitespace-nowrap ${category === cat ? 'bg-green-600 text-white' : 'border-gray-400 text-gray-700'}`}
          >
            {cat}
          </button>
        ))}
              ...
              {inCart ? (
                <>
                  <div className="mt-2 flex justify-center items-center gap-2">
                    <button onClick={() => decreaseQty(product.id)} className="px-3 py-1 border border-gray-400 text-gray-700 rounded-full hover:bg-gray-100">-</button>
                    <span>{inCart.quantity}</span>
                    <button onClick={() => increaseQty(product.id)} className="px-3 py-1 border border-gray-400 text-gray-700 rounded-full hover:bg-gray-100">+</button>
                  </div>
                  <Link href="/cart" className="inline-block mt-2 border border-blue-600 text-blue-600 px-3 py-1 rounded-full hover:bg-blue-50">Go to Cart</Link>
                </>
              ) : (
                <button
